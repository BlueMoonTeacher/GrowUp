...
const AssessmentPreviewModal = ({ 
    ...
    const domainInputClass = "w-full min-h-[3.5rem] resize-none overflow-hidden text-center bg-white text-gray-900 border border-gray-300 focus:outline-none focus:ring-1 focus:ring-primary rounded px-2 py-1 text-sm leading-tight [align-content:center]";
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div 
                className="bg-white rounded-xl shadow-2xl w-full max-w-[min(96vw,1400px)] max-h-[90vh] flex flex-col border border-base-300" 
            ...
            >
                ...
                <div className="flex-1 overflow-auto custom-scrollbar p-4 bg-base-100">
                    ...
                    <table className="w-full min-w-[1120px] text-sm border-collapse table-fixed">
                        <thead>
                            <tr className="bg-base-200 text-left text-gray-700 font-bold">
                                ...
                                <th className="p-2 border w-56 text-center">영역</th>
                        ...
                        <tbody>
                            {editableData.map((item, index) => (
                                <tr key={index} className="hover:bg-base-50">
                                    ...
                                    <td className="p-1 border bg-white align-top">
                                        <textarea 
                                            value={item.domain} 
...
const EvaluationEntry = ({ 
    ...
    const studentSections = useMemo(() => {
        ...
        const girls = students.filter(s => String(s.gender || '').trim().startsWith('여'));
        const others = students.filter(s => {
            const gender = String(s.gender || '').trim();
            return !gender || (!gender.startsWith('남') && !gender.startsWith('여'));
        });

        return [
            { title: '남학생', students: boys },
            { title: '여학생', students: girls },
            ...(others.length > 0 ? [{ title: '성별 미지정', students: others }] : [])
    ...
    return (
        <div className="h-full flex flex-col p-2 sm:p-4 gap-4 overflow-hidden relative">
            ...
            <div className="bg-base-50 p-4 rounded-xl border border-base-300 flex flex-col gap-3 shrink-0">
                ...
                {subjects.length > 0 && (
                    <div className="grid grid-cols-1 xl:grid-cols-[minmax(260px,auto)_minmax(0,1fr)] gap-3 items-end">
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg border text-xs font-bold transition-all ${
                                            selectedDomain === d
                                                ...
                                                : 'bg-white text-gray-600 border-base-300 hover:border-primary/50 hover:bg-green-50'
                                        }`}
                                    >
                                        {d}
                                    </button>
            ...
            {selectedAssessment ? (
                <div className="flex-1 flex flex-col lg:flex-row gap-4 overflow-hidden min-h-0 relative">
                    <div className="flex-1 bg-white border border-base-300 rounded-xl flex flex-col shadow-sm overflow-hidden min-h-0 relative">
                        ...
                        <div className="flex-1 overflow-auto custom-scrollbar relative bg-white p-3 pb-40">
                            <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
                                {studentSections.map(section => {
                                    const sectionTone = section.title === '남학생'
                                    ...
                                    return (
                                    <div key={section.title} className={`rounded-xl border bg-white overflow-hidden shadow-sm ${sectionTone.split(' ').find(c => c.startsWith('border-')) || 'border-gray-300'}`}>
                                        <div className={`flex items-center justify-between px-3 py-2 border-b ${sectionTone}`}>
                                            <span className="text-sm font-bold text-base-content">{section.title}</span>
                                            <span className="text-[11px] font-bold text-base-content-secondary">{section.students.length}명</span>
                                        </div>
                                        <div className="grid grid-cols-1 2xl:grid-cols-2 bg-white">
                                            {section.students.map(student => {
                                                const level = getLevel(student.id);
                                                const isChanged = unsavedChanges[student.id] !== undefined;

                                                return (
                                                    <div key={student.id} className={`grid grid-cols-[2rem_minmax(3rem,1fr)_auto] items-center gap-1.5 px-2 py-1.5 border-b border-gray-200 odd:bg-white even:bg-gray-50/50 2xl:odd:border-r 2xl:odd:border-r-gray-200 2xl:[&:nth-last-child(-n+2)]:border-b-0 transition-colors hover:bg-green-50 ${isChanged ? '!bg-blue-50' : ''}`}>
                                                        <div className="text-center text-xs font-bold text-gray-500">{student.number}</div>
                                                        <div className="truncate text-sm font-bold text-gray-800">
                                                            {student.name.hangul}
                                                            {isChanged && <span className="text-[9px] text-blue-500 ml-1">●</span>}
                                                        </div>
                                                        <div className="flex justify-end gap-0.5">
                                                            {(['high', 'middle', 'low'] as const).map(lvl => (
                                                                <button
                                                                    ...
                                                                    className={`w-9 h-7 rounded-md border transition-all font-bold text-[10px] shadow-sm ${
                                                                        level === lvl 
                                                                        ? lvl === 'high' ? 'bg-green-500 text-white border-green-600 ring-1 ring-green-200'
                                                                        : lvl === 'middle' ? 'bg-yellow-400 text-white border-yellow-500 ring-1 ring-yellow-200'
                                                                        : 'bg-red-400 text-white border-red-500 ring-1 ring-red-200'
                    ...
                    <div className="w-full lg:w-80 flex flex-col gap-3 overflow-y-auto custom-scrollbar shrink-0 min-h-0 pb-40 lg:pb-0">
                         {/* Stats Panel */}
                         <div className="bg-white p-3 rounded-xl border border-base-300 shadow-sm shrink-0">
                             <h4 className="font-bold mb-2 text-sm text-base-content-secondary flex justify-between">
                                 진행 현황 <span className="text-[10px] font-normal text-gray-400 self-center">{stats.done}/{stats.total}명</span>
                             </h4>
                             ...
                             <div className="grid grid-cols-5 gap-1.5 text-center text-[10px]">
                                 ...
                                 <div className="bg-yellow-50 py-1.5 rounded border border-yellow-100">
                                     <div className="font-bold text-yellow-700 leading-none">{stats.m}</div>
                                     <div className="text-yellow-600 mt-0.5">보통</div>
                                 </div>
                                 <div className="bg-red-50 py-1.5 rounded border border-red-100">
                         ...
                         <div className="bg-white p-4 rounded-xl border border-base-300 shadow-sm overflow-hidden flex-1 min-h-[220px] flex flex-col">
                             <h4 className="font-bold mb-3 text-base-content flex items-center gap-2">
                                 <span>ℹ️</span> 평가 기준
                             </h4>
                             <div className="space-y-2 text-xs flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-1 pb-8">
                                 ...
                                 <div>
                                     ...
                                     <p className="mt-1 p-2 bg-green-50/60 rounded border border-green-100 text-gray-800 leading-relaxed whitespace-pre-wrap">{selectedAssessment.criteria.high || '-'}</p>
                                 </div>
                                 <div>
                                     <span className="text-xs font-bold bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">보통</span>
                                     <p className="mt-1 p-2 bg-yellow-50/60 rounded border border-yellow-100 text-gray-800 leading-relaxed whitespace-pre-wrap">{selectedAssessment.criteria.middle || '-'}</p>
                                 </div>
                                 <div>
                                     <span className="text-xs font-bold bg-red-100 text-red-800 px-2 py-0.5 rounded">노력요함</span>
                                     <p className="mt-1 p-2 bg-red-50/60 rounded border border-red-100 text-gray-800 leading-relaxed whitespace-pre-wrap">{selectedAssessment.criteria.low || '-'}</p>
                                 </div>
                             </div>
                         </div>
                    </div>
