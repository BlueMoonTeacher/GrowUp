...
export const ATTENDANCE_SYMBOL_COLOR_CLASSES: Record<AttendanceCategory, string> = {
  '질병': 'text-amber-600',
  '미인정': 'text-rose-500',
  '기타': 'text-violet-600',
  '인정': 'text-emerald-500',
...
export const getAttendanceSymbolColorClass = (category: AttendanceCategory): string => {
  return ATTENDANCE_SYMBOL_COLOR_CLASSES[category] || 'text-gray-700';
};
...
export const CATEGORIES: { category: AttendanceCategory; label: string; color: string }[] = [
  ...
  { category: '기타', label: '기타', color: 'text-violet-700 bg-violet-50 border-violet-200' },
